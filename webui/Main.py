...
webui_log_file = os.path.join(log_dir, "webui.log")
...
def open_task_folder(task_id):
    try:
        ...
        raw = str(task_id)
        # Accept either a legacy UUID or the new `task<N>_YYYYMMDD_HHMMSS`
        # style; both are restricted to a safe character set so the final
        # path can never escape the tasks root.
        try:
            safe = str(UUID(raw))
        except (ValueError, AttributeError):
            if not re.fullmatch(r"task\d+_\d{8}_\d{6}", raw):
                logger.warning(f"refusing to open task folder with unsafe name: {raw!r}")
                return
            safe = raw

        tasks_root = os.path.abspath(os.path.join(root_dir, "storage", "tasks"))
        path = os.path.abspath(os.path.join(tasks_root, safe))
...
def read_recent_log_lines(file_path: str, max_lines: int = 400) -> str:
    ...
    with st.container(border=True):
        ...
        if params.video_source == "local":
            ...
            if new_drop_dir != current_drop_dir:
                config.app["material_directory"] = new_drop_dir
                # Persist immediately so the value survives Streamlit reruns.
                config.save_config()
...
with st.container(border=True):
    ...
    params.target_minutes = float(target_minutes)
    config.app["long_form"] = params.long_form
...
if start_button:
    config.save_config()
    task_id = utils.next_task_id()
    if not params.video_subject and not params.video_script:
        st.error(tr("Video Script and Subject Cannot Both Be Empty"))
        scroll_to_bottom()
        st.stop()

    allowed_sources = ["pexels", "pixabay", "coverr", "videvo", "mixkit", "local"]
