...
def preprocess_video(materials: List[MaterialInfo], clip_duration=4):
    ...
    # The user can also configure a "drop folder" via `material_directory` in
    # config.toml. Files under that folder are treated as trusted local material
    # so that purchased Shutterstock/Adobe/Getty clips can be reused without
    # re-uploading them through the WebUI each time.
    from app.config import config as _config  # local import to avoid circular deps
    extra_dir = (_config.app.get("material_directory") or "").strip()
    extra_dir_real = os.path.realpath(extra_dir) if extra_dir else ""

    def _resolve_local_material_path(url: str) -> str:
        # Try the default WebUI upload dir first, then the optional drop folder.
        last_error = None
        for base in (local_videos_dir, extra_dir_real):
            if not base:
                continue
            try:
                return file_security.resolve_path_within_directory(base, url)
            except ValueError as exc:
                last_error = exc
        if last_error is not None:
            raise last_error
        raise ValueError("no local material base directory available")
    ...
    for material in materials:
        ...
        try:
            material_source_path = _resolve_local_material_path(material.url)
        except ValueError as exc:
            ...
            logger.warning(
                ...
                f"material_directory: {extra_dir_real or '(unset)'}, "
                f"error: {str(exc)}"
            )
            continue
