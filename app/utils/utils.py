...
def next_task_id() -> str:
    """
    Generate a human-readable task id of the form ``task<N>_<YYYYMMDD>_<HHMMSS>``,
    where ``<N>`` is one greater than the highest task number currently present
    under ``storage/tasks``. The datetime suffix makes collisions effectively
    impossible even if the counter is reset.
    """
    import re
    ...
    return f"task{next_num}_{stamp}"
